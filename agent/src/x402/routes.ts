...
export function setRouteWallet(w: AgentWallet) {
  walletRef = w;
}
...
export function registerRoutes(app: Express, orchestrator: JobOrchestrator) {
  ...
  app.post('/api/pinata-upload', (req, res) => {
    try {
      ...
      const bytesPerFile =
        fileCount > 0 ? Math.floor(totalBytes / fileCount) : 0;
      ...
      res.json({
        logged: fileCount,
        pinataUsage: costTracker.getPinataUsage(),
      });
    } catch (err: any) {
      res.status(500).json({ error: err.message });
    }
  ...
  app.post('/api/ai/analyze-image', async (req, res) => {
    try {
      ...
      costTracker.logCost({
        type: 'openai',
        amount_usd: 0.01,
        details: 'ai-service-analyze',
      });
      orchestrator.emit('action', {
        type: 'ai_service_sold',
    ...
    try {
      ...
      const result = await openai.chat.completions.create({
        ...
        messages: [
          {
            role: 'user',
            content: `Classify this text into one of these categories: ${categories?.join(', ') || 'positive, negative, neutral'}.\n\nText: "${text}"`,
          },
        ],
  ...
  app.post('/api/ai/verify-photo', async (req, res) => {
    try {
        ...
        messages: [
          {
            ...
            content: [
              {
                type: 'text',
                text: `Verify this photo meets the following requirements: ${requirements}. Respond as JSON with { "meets_requirements": true/false, "reasoning": "...", "score": 0.0-1.0 }`,
              },
              { type: 'image_url', image_url: { url: imageUrl } },
            ],
          },
      ...
      costTracker.logCost({
        type: 'openai',
        amount_usd: 0.01,
        details: 'ai-service-verify',
      });
      orchestrator.emit('action', {
        type: 'ai_service_sold',
        service: 'verify-photo',
        revenue: 0.04,
        timestamp: Date.now(),
      });
      orchestrator.emitMetrics();
  ...
  app.get('/api/worker/:address/reputation', async (req, res) => {
    try {
      ...
      res.json({
        ...rep,
        tier: getReputationTier(rep.reputation_score, rep.tasks_completed),
      });
    } catch (err: any) {
      res.status(500).json({ error: err.message });
    }
  ...
  app.get('/api/tasks/recommended/:address', async (req, res) => {
    try {
      ...
      const openTaskIds: string[] = req.query.taskIds
        ...
        : [];
      const recommendations = await getRecommendedTasks(
        workerTags,
        openTaskIds,
      );
